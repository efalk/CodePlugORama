...
database is in this format. See
https://www.wwara.org/coordinations/coordination-data-files/ for
more information. This file format is written for repeaters, and so rxfreq and txfreq
...
This is the format used by NERD to publish repeater lists. See
https://www.nerepeaters.com for more information. This file format
is written for repeaters, and so rxfreq and txfreq are swapped.
This tool swaps them back before writing the output.
...
|COUNTY|Middlesex|
|?|Not clear; usually blank, sometimes a number like "4136"|
|VOIP|e.g. "E 4260", "A 2350", etc. Usually blank|
|LINKS/COMMENTS|Any text, may contain input frequency if OFFSET is '*'|
...
NXDN, and DMR. In cases like this, one input record can result in
multiple output records.

Additional information such as the dmr color code or nxdn radio access number will
be put into the txtone field of the output.
...
ICS-217 is the standard FEMA form for distributing frequency assignments to emergency
responders. The exact format depends on the organization.

The format used by this tool is specific to the [Seattle ACS](https://www.seattleacs.org/).
It may work with other organizations' ICS-217 forms but this has
not been tested. Contact KK7NNS at gmail if you need more information.
